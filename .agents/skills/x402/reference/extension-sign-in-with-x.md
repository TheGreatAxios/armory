...
Uses the `SIGN-IN-WITH-X` header as shown above.

### MCP Transport

Include in the initial tool call via `_meta`:
...
```json
{
  ...
  "params": {
    ...
    "arguments": {},
    "_meta": {
      "sign-in-with-x": <base64-encoded SIWX message>
    }
  }
}
```
...
Include in task message metadata:
...
```json
{
  ...
  "params": {
    "message": {
      ...
      "metadata": {
        "x402.sign-in-with-x": <base64-encoded SIWX message>
      }
    }
  }
}
...
- [Coinbase x402 Sign-In-With-X Extension](https://github.com/coinbase/x402/blob/main/specs/extensions/sign-in-with-x.md)
