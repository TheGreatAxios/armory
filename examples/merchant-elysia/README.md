...
Example merchant server using ElysiaJS with X-402 payment middleware. Demonstrates how to protect API routes with payment requirements.
...
- **ElysiaJS Integration** - Type-safe middleware with schema validation
...
**Without payment:**
```bash
curl http://localhost:3004/api/premium
...
```bash
curl -X POST http://localhost:3004/api/purchase \
...
```bash
curl http://localhost:3004/api/user/profile \
  -H "PAYMENT-SIG: <signed_payment_payload>"
```
...
const app = new Elysia()
  ...
  .get("/api/protected", ({ store }) => {
    // Payment info attached to store
    const payment = store.payment as PaymentInfo;
    return { message: `Hello ${payment.payerAddress}` };
  });
```

## Schema Validation

ElysiaJS provides built-in schema validation:
...
ElysiaJS middleware provides full TypeScript support with typed context:
...
const app = new Elysia<{ store: PaymentStore }>()
  .use(elysiaPaymentMiddleware({ ... }))
  .get("/api/protected", ({ store }) => {
    // store.payment is fully typed
    const { payerAddress, verified } = store.payment;
    // ...
  });
```
...
| - Decode & validate     |
| - Verify payment        |
| - Attach store.payment  |
+-------------------------+
       |
       v
+-------------+
| Route       | <--- store.payment available
| Handler     |
+-------------+
```
