/**
 * E2E Test: Armory Client → Coinbase Hono Server → PayAI Facilitator
 * 
 * Run: bun test tests/e2e/armory-to-coinbase-hono.test.ts
 * 
...
// ============================================================================
// Coinbase Hono Server (simplified x402-hono format)
// ============================================================================

function createCoinbaseHonoServer(facilitatorUrl: string) {
  return serve({
    port: 0,
    async fetch(req) {
...
describe("Armory Client → Coinbase Hono Server → PayAI", () => {
  test("server returns 402 with PayAI facilitator", async () => {
    const server = createCoinbaseHonoServer(FACILITATORS.payai);
    ...
    try {
      ...
      expect(decoded.accepts[0].facilitator).toBe(FACILITATORS.payai);
    } finally {
      server.stop();
    }
  });
