...
Use the version of the Express package that matches your framework:

- **Express v5+:** `@armory-sh/middleware-express` (shown in the first tab)
- **Express v4:** `@armory-sh/middleware-express-v4` (shown in the second tab)
...
Both tabs show the same protection flow; use the v4 package if you are still on Express 4 and the core package once you upgrade to Express 5+.
...
```typescript
{
  facilitatorUrl: string  // Required for payment verification
}
```
