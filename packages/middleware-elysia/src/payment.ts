...
export const paymentMiddleware = (config: PaymentConfig) => {
  ...
  return new Elysia({ name: "armory-payment" })
    .derive(() => ({
      payment: undefined as { payload: X402PaymentPayload; payerAddress: string; verified: boolean } | undefined,
    }))
    .onBeforeHandle(async (context) => {
      if (error) {
        return errorResponse(
          { error: "Payment middleware configuration error", details: error.message },
          500
        );
      }
      ...
      if (!primaryRequirement) {
        return errorResponse({ error: "Payment middleware configuration error", message: "No payment requirements configured" }, 500);
      }
      ...
      const verifyResult: VerifyResponse = await verifyPayment(paymentPayload, primaryRequirement, { url: facilitatorUrl });

      if (!verifyResult.isValid) {
        return errorResponse(
          { error: "Payment verification failed", message: verifyResult.invalidReason },
          402,
          createPaymentRequiredHeaders(requirements)
        );
      }
      ...
      (context as { payment?: { payload: X402PaymentPayload; payerAddress: string; verified: boolean } }).payment = { payload: paymentPayload, payerAddress, verified: true };
    })
    .onAfterHandle(async (context) => {
      ...
      const settleResult: X402SettlementResponse = await settlePayment(payment.payload, primaryRequirement, { url: facilitatorUrl });

      if (!settleResult.success) {
        return errorResponse({ error: "Settlement failed", details: settleResult.errorReason }, 502);
      }
