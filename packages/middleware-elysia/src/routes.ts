...
export const routeAwarePaymentMiddleware = (
  ...
  const { routes: resolvedRoutes, error: configError } =
    resolveRouteConfig(config);
  ...
  return new Elysia({ name: "armory-route-aware-payment" })
    ...
    .onBeforeHandle(async (context) => {
      try {
        ...
        const payerAddress =
          verifyResult.payer ?? payload.payload.authorization.from;
        (context as { payment?: RouteAwarePaymentInfo }).payment = {
          payload,
          payerAddress,
          verified: true,
    ...
    .onAfterHandle(async (context) => {
      ...
      const matchedRoute = payment.route
        ? resolvedRoutes.find((route) => route.pattern === payment.route)
        : undefined;
      const routeConfig = matchedRoute?.config;
      if (!routeConfig) {
        return;
      ...
      const settlementResult: X402SettlementResponse = await settlePayment(
        payment.payload,
